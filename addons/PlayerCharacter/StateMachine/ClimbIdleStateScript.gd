...
func enter(charRef : Variant):
	...
	cR.wallNormal = cR.climbWallNormal
...
func exit():
	cR.wallAnchor.reparent(cR)
	cR.wallAnchor.position = Vector3(0,1,0)
	cR.wallAnchor.linear_velocity = Vector3.ZERO
...
func update_wall_anchor():
	cR.wallAnchor.reparent(cR.wallCheck.get_collider())
	cR.wallAnchor.global_position = cR.wallCheck.get_collision_point() + cR.wallNormal * 0.5
...
func physics_update(delta : float):
	...
	var new_pos = cR.wallAnchor.global_position
	new_pos.y -= 1.55
	cR.global_position = new_pos
...
func inputManagement():
	if Input.is_action_just_pressed(cR.ropeAction) or cR.isRopeAbilityRequested:
...
func move(delta : float):
	...
	if cR.moveDirection:
		...
		cR.wallAnchor.linear_velocity = lerp(cR.velocity, cR.moveDirection * cR.moveSpeed, cR.moveAccel * delta)
	else:
		#apply smooth stop 
		cR.velocity = lerp(cR.velocity, Vector3.ZERO, cR.moveDeccel * delta)
		cR.wallAnchor.linear_velocity = lerp(cR.velocity, Vector3.ZERO, cR.moveDeccel * delta)
