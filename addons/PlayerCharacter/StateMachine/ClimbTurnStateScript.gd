...
class_name ClimbTurnState

var stateName : String = "ClimbTurn"
...
func _compute_wall_local_axes():
	...
	_try_update_detectors_orientation()
...
func _try_update_detectors_orientation():
	var dir = cR.wallDetector.global_position - cR.wallNormal
	cR.wallDetectorTmp.look_at(dir, Vector3.UP)
	if cR.wallCenterCheckTmp.is_colliding():
		if cR.wallCenterCheckTmp.get_collision_normal() == cR.wallNormal:
...
func move(delta : float):
	...
	if (cR.moveLeftAction and cR.wallLeftCheck):
		transitioned.emit(self, "ClimbTurnState")
	
	if (cR.moveRightAction and cR.wallRightCheck):
		transitioned.emit(self, "ClimbTurnState")
