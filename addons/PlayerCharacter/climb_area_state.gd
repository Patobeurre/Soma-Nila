...
class_name ClimbAreaState

var stateName : String = "ClimbArea"

var cR : CharacterBody3D

func enter(charRef : Variant):
	cR = charRef as CharacterBody3D
	cR.wallNormal = cR.climbAreaWallNormal
	
	verifications()


func exit():
	cR.climbingAreas.clear()
	cR.isClimbAreaAbilityRequested = false
...
func move(delta : float):
	...
	var clampedX = cR.inputDirection.x if abs(cR.inputDirection.x) > 0.3 else 0
	cR.moveDirection = (cR.wallSideDir * clampedX).normalized()
